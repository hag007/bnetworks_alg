...
cat(paste('License: ','\n'))
cat(paste('Copyright (c) 2015 Leibniz-Institut für Naturstoff-Forschung und Infektionsbiologie e.V. - Hans-Knoell-Institut (HKI) and Friedrich-Schiller-Universität Jena.','\n','written by Sebastian Vlaic - Sebastian.Vlaic@hki-jena.de','\n','\n','This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation, Version 3.','\n','\n','This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.','\n','\n'))

showRequiredLibraries <- function(){
	cat(paste('the following libraries are required by ModuleDiscoverer:','\n'))
	cat(paste('\t','igraph','\n'))
	cat(paste('\t','parallel','\n'))
	cat(paste('\t','doParallel','\n'))
	cat(paste('\t','foreach','\n'))
	cat(paste('\t','Cairo','\n','\n'))
}

cat(paste('loading required libraries: '))
if(!library('igraph', logical.return=TRUE, quietly=TRUE, warn.conflicts=FALSE)){
	cat(paste('ERROR loading igraph library','\n'))
	showRequiredLibraries()
	stop()
}
if(!library('parallel', logical.return=TRUE, quietly=TRUE, warn.conflicts=FALSE)){
	cat(paste('ERROR loading parallel library','\n'))
	showRequiredLibraries()
	stop()
}
if(!library('doParallel', logical.return=TRUE, quietly=TRUE, warn.conflicts=FALSE)){
	cat(paste('ERROR loading doParallel library','\n'))
	showRequiredLibraries()
	stop()
}
if(!library('foreach', logical.return=TRUE, quietly=TRUE, warn.conflicts=FALSE)){
	cat(paste('ERROR loading foreach library','\n'))
	showRequiredLibraries()
	stop()
}
if(!library('Cairo', logical.return=TRUE, quietly=TRUE, warn.conflicts=FALSE)){
	cat(paste('ERROR loading Cairo library','\n'))
	showRequiredLibraries()
	stop()
}
cat(paste("OK","\n"))
...
moduleDiscoverer.db.create_MD_object <- function(database=NULL, foregrounds=NULL, background=NULL, cores=1, chunks=5000, randomDataSets=NULL, minBackgroundRequired=3, minBackgroundToCliqueRatio=0.5, minForegroundRequired=1){
	if(is.null(database)){
	...
	}else if(is.null(randomDataSets)){
		cat(paste("INFO: no randomDataSets provided. Using one-sided Fisher's exact test instead.",'\n'))
	}
	...
	if(detectCores()<=cores){
		...
		cat(paste('\t',"WARNING: Maximal number of cores detected on the system is ",detectCores(),". Using ",runCores," cores instead!",'\n',sep=""))
	}
	...
	return(list("foregrounds"=foregrounds,"background"=background,"randomDataSets"=randomDataSets,"cores"=cores,"relevantCliques"=relevantCliques,"totalCliques"=total,"cores"=cores,"chunks"=chunks,"numberOfForegrounds"=numberOfForegrounds,"numberOfRandomDataSets"=numberOfRandomDataSets,"permutationBasedPvalueCalculation"=!is.null(randomDataSets)))
}
...
moduleDiscoverer.db.testForCliqueEnrichment <- function(database=NULL, input=NULL, cores=NULL, chunks=NULL){
	...
	if(input$permutationBasedPvalueCalculation==TRUE){
	...
	return(list("foregrounds"=foregrounds, "background"=background, "randomDataSets"=randomDataSets, "p.value"=p.value, "cores"=cores, "numberOfForegrounds"=numberOfForegrounds, "numberOfRandomDataSets"=numberOfRandomDataSets, "relevantCliques"=relevantCliques, "permutationBasedPvalueCalculation"=input$permutationBasedPvalueCalculation, "chunks"=chunks))
...
moduleDiscoverer.db.computeNetworkStability <- function(database=NULL, result=NULL, repeats=100, p.value=0.05, chunks=NULL, cores=NULL, size=NULL, module.reference=NULL){
	...
	bootstrap.samples <- foreach(k = 1:repeats, .combine = 'append') %dopar% {
		bs <- sample(x=1:total, replace=TRUE, size=size)
		#now, in order to re-use the already calculated p-values we have to make sure that the statistical background
		#has not changed. In other words, we have to check if the selected cliques cover all proteins that were part of
		#a clique in the original clique-database.
		if(!all(database$proteinsInClique %in% as.numeric(unique(unlist(strsplit(database$uniqueCliqueId[database$results[database$results[,"run"] %in% bs,"id"]], split=" ")))))){
			return(list(NULL))
		}else{
			return(list(bs))
		}
	...
	if(any(sapply(bootstrap.samples, is.null))){
		if(all(sapply(bootstrap.samples, is.null))){
			cat(paste("ERROR: no bootstrap samples were created! Increase the size of the sample!",'\n',sep=""))
			cat("INFO: stopping parallel environment...")
				stopCluster(cl)
			cat(paste("done!",'\n'))
			stop()
		}else{
			cat(paste("WARNING: could only create ",sum(!sapply(bootstrap.samples, is.null))," valid bootstrap samples... your results will rely only on them!",'\n',sep=""))
			repeats <- sum(!sapply(bootstrap.samples, is.null))
			bootstrap.samples <- lapply(which(!sapply(bootstrap.samples, is.null)), function(i){return(bootstrap.samples[[i]])})
		}
	}
...
moduleDiscoverer.db.plotComputedPValues <- function(result=NULL, fileName=NULL, p.value=0.05){
	...
	for(i in 1:length(result$p.value)){
		if(result$permutationBasedPvalueCalculation){
			...
			plot(result$p.value[[i]][o,2], pch=16, cex=1, type="p", col="green", xlab="rank", ylab=paste(coef.name,"p-value",sep="\n"), ylim=c(0,max(p.value,max(result$p.value[[i]][result$p.value[[i]][,1]<0.01,2]))), xlim=c(0, which(result$p.value[[i]][o,1]>=p.value)[1]-1), main=paste("ranked p-values (0 - ",p.value,")",sep=""))
			points(result$p.value[[i]][o,1], pch=16, cex=1, lwd=2, type="l", lty=2, col="blue")
			legend("topleft", legend=c("Fisher's exact test","permutation based"), lty=c(NA,2), pch=c(16,NA), col=c("green","blue"))
		}else{
		}
...
moduleDiscoverer.module.createModule <- function(result=NULL, module.name="ModuleDiscoverer - regulatory module", cores=NULL){
	...
	if(length(result$enrichedCliques)>0){
		...
		genes <- unique(unlist(result$enrichedCliques))
		cat(".")
		m <<- matrix(0, ncol=length(genes), nrow=length(genes), dimnames=list(genes,genes))
		cat(".")
		tmp <- lapply(result$enrichedCliques, function(x){
			m[x,x] <<- matrix(1, ncol=length(x), nrow=length(x))
			return(NULL)
		})
		cat(".")
		rm(tmp)
		rm(genes)
		gc()
		g = igraph::simplify(graph.adjacency(adjmatrix=m, mode="undirected"))
		cat(".")
